  ...
  public static class Mediaportal
  {
    ...
    public static MediaPortalVersion GetVersion()
    {
      var minimumVersions = new Dictionary<MediaPortalVersion, Version>()
            {
                { MediaPortalVersion.MP1_33, new Version(1, 32, 100) },  // Let's assume the pattern continues
    ...
    public static string GetMediaPortalPath()
    {
      ...
      if (mpdir == null)
      {
        ...
        {
          mpdir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Team MediaPortal", "MediaPortal");
          if (!Directory.Exists(mpdir))
          {
            mpdir = null;
          }
    ...
    public static string GetServerPath()
    {
      ...
      if (mpdir == null)
      {
        ...
        if (!Directory.Exists(mpdir))
        {
          mpdir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Team MediaPortal", "MediaPortal TV Server");
          if (!Directory.Exists(mpdir))
          {
            mpdir = null;
          }
