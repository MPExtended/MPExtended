...
{
  #region Exif Read Wrapper
  internal static class EXIFInfoWrapper
  {
    public static WebEXIFInfo GetExifInfo(MediaSource source)
    {
      if (source.MediaType != WebMediaType.Picture)
      {
        return null;
      }
      ...
      using (var context = source.CreateNetworkContext())
      {
        ...
        if (!source.Exists)
        {
          Log.Warn("Trying to load EXIF for {0}, which does not exist or is inaccessible", source.GetDebugName());
          return null;
        }
        else if (!source.SupportsDirectAccess)
        {
          Log.Warn("Loading EXIF for non-direct access source {0} isn't supported yet", source.GetDebugName());
          return null;
        }

        WebEXIFInfo info = new WebEXIFInfo();
        using (ExifMetadata extractor = new ExifMetadata())
        {
          Metadata exif = extractor.GetExifMetadata(context.RewritePath(source.GetPath()));
          if (!exif.IsEmpty())
          {
      ...
      try
      {
        ...
        if (MyMetadata.Resolution.IsEmpty || MyMetadata.ImageDimensions.IsEmpty)
        {
          try
          {
            ...
            Images.GetImageSizes(photoName, out MyMetadata.Resolution, out MyMetadata.ImageDimensions);
          }
          catch
          {
