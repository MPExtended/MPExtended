...
namespace MPExtended.PlugIns.MAS.MPTVSeries
{
  ...
  public partial class MPTVSeries : Database, ITVShowLibrary
  {
    ...
    public MPTVSeries(IPluginData data)
    {
      ...
      this.fanartconfiguration = data.GetConfiguration("FanartHandler");
    ...
    private LazyQuery<T> GetAllTVShows<T>() where T : WebTVShowBasic, new()
    {
      ...
      return new LazyQuery<T>(this, sql, new List<SQLFieldMapping>() {
        ...
        }, delegate (T item)
        {
          ...
          {
            actor.Artwork = GetArtworkForActor(actor.Title);
          }
          return item;
        });
    }
    ...
    public WebTVShowGenre GetGenreById(string id)
    {
      return GetAllGenres().Where(x => x.Title == id).First();
    }

    public IEnumerable<WebTVShowGenre> GetAllGenres()
    {
      ...
      return ReadList<IEnumerable<string>>(sql, delegate (SQLiteDataReader reader)
          {
            return reader.ReadPipeList(0);
              ...
              .Distinct()
              .OrderBy(x => x)
              .Select(x => new WebTVShowGenre() { Title = x, Artwork = GetArtworkForGenre(x) });
    }

    public WebTVShowActor GetActorById(string id)
    {
      return GetAllTVShowsBasic().AsQueryable().SelectMany(x => x.Actors).Distinct().Where(x => x.Title == id).First();
    }
    ...
    public List<WebArtwork> GetArtworkForActor(string title)
    {
      ...
      // Poster
      int i = 0;
      string sql = @"SELECT Pretty_Name || '\' || REPLACE(Image, '/', '\') FROM Actors a LEFT JOIN online_series s ON a.SeriesId = s.ID WHERE a.Name = '" + title + "' AND TRIM(a.Image) <> '';";
      string[] files = ReadList<IEnumerable<string>>(sql, delegate (SQLiteDataReader reader)
      {
        return reader.ReadPipeList(0);
      }).SelectMany(x => x)
        .Distinct()
        .ToList()
        .Select(x => Path.Combine(configuration["banner"], x))
    ...
    private List<WebArtwork> GetArtworkForGenre(string title)
    {
      ...
      // Poster
      int i = 0;
      string folder = Path.Combine(fanartconfiguration["thumb"], "Videos", "Genre", title + "{0}");
    ...
    {
      if (type == WebMediaType.TVShowActor)
      {
        return new WebDictionary<string>()
                {
                    { "Type", "mptvseries actor" },
                    { "Id", GetActorById(id).Title }
                };
      }

      if (type == WebMediaType.TVShowGenre)
      {
        return new WebDictionary<string>()
                {
                    { "Type", "mptvseries genre" },
