...
namespace MPExtended.Services.MediaAccessService.Code
{
    internal class MovingPictures : Database
    {
        ...
        public WebMovieFull GetFullMovie(int idMovie)
        {
            ...
            movie.Files = new List<WebMovieFull.WebMovieFile>();
        ...
        public List<WebMovieFull> GetAllMoviesDetailed()
        {
            string sql = "SELECT movie.id, local.fullpath, local.discid, local.filehash, local.part, local.duration, local.videowidth, local.videoheight, local.videoresolution, local.videocodec, " +
                         "local.audiocodec, local.audiochannels, local.hassubtitles, local.videoformat, " +
                         "movie.title, movie.alternate_titles, movie.sortby, movie.directors, movie.writers, movie.actors, " +
                         ...
                         "INNER JOIN local_media AS local ON local.id = info.local_media_id AND local.ignored = 0 " +
                         "ORDER BY movie.id";
            Dictionary<int, WebMovieFull> ret = new Dictionary<int, WebMovieFull>();
            ReadList<int>(sql, delegate(SQLiteDataReader reader)
            {
                int id = DatabaseHelperMethods.SafeInt32(reader, 0);
                if (!ret.ContainsKey(id))
                {
                    ret[id] = new WebMovieFull();
                }
        ...
        private WebMovieFull AddToMovie(SQLiteDataReader reader, WebMovieFull movie)
        {
            try
            {
        ...
        public List<WebMovie> SearchForMovie(String searchString)
        {
            string sql = "SELECT DISTINCT movie.id, movie.title, movie.tagline, movie.year, movie.genres, movie.coverthumbfullpath, movie.backdropfullpath " +
                         ...
                         "ORDER BY movie.id ";
            return ReadList<WebMovie>(sql, FillBasicMovie, null, null, new SQLiteParameter("@search", String.Format("%{0}%", searchString)));
        }
        ...
        public string GetMoviePath(int itemId, int part)
        {
            try
            {
                WebMovieFull movie = GetFullMovie(itemId);
